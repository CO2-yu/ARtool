...
- マーカーを見失った場合は、設定された喪失猶予時間を超えた後に該当モデルを非表示にする。
- マーカー喪失猶予時間の初期値は800msとする。
- マーカー喪失猶予時間は `app.config.json` の `lostTimeoutMs` で変更できる。
- `lostTimeoutMs` の推奨範囲は500msから1500msとする。
- マーカー喪失後の空間固定ARは行わない。
...
- 初期表示が1/12スケールになる。
- 複数マーカーを最大3件まで扱える。
- マーカー喪失猶予中は最後に認識できた位置でモデル表示を維持する。
- マーカー喪失猶予時間を超えるとモデルが非表示になる。
- ロード中表示が出る。
- 静止画撮影ができる。
